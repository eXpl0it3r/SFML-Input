...
m_triangles{sf::PrimitiveType::Triangles, static_cast<std::size_t>(sf::Keyboard::Scan::ScancodeCount) * 6},
m_labels(static_cast<std::size_t>(sf::Keyboard::Scan::ScancodeCount), sf::Text{font, "", 16})
{
    ...
    // Check all the scancodes are in the matrix exactly once
    {
        ...
        assert(scancodesInMatrix.size() == static_cast<std::size_t>(sf::Keyboard::Scan::ScancodeCount));
    }
